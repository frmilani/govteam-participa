# Story: Campanhas WhatsApp (RF-007)
...
## Visão Geral
Esta funcionalidade é o core de engajamento do sistema. Ela permite transformar os Leads capturados em participantes ativos das enquetes através de mensagens personalizadas enviadas via WhatsApp (Evolution API), rastreando cada interação individualmente.

## Critérios de Aceite
- [x] Criar campanha através de um wizard guiado.
- [x] Segmentar leads por tags com preview de contagem.
- [x] Suportar templates de mensagem com merge fields (`{{nome}}`, `{{link_unico}}`).
- [x] Gerar Tracking Links únicos e curtos para cada lead.
- [x] Enviar mensagens via integração com Evolution API (BullMQ implemented).
- [x] Agendar disparos para datas futuras (BullMQ delay trigger).
- [x] Monitorar estatísticas em tempo real (Painel de Monitoramento implemented).

## Lista de Arquivos (A serem criados/modificados)
- `src/lib/campanhas/tracking-service.ts`: Geração e validação de hashes.
- `src/lib/campanhas/campanha-service.ts`: Orquestração de envios e status.
- `src/lib/whatsapp/whatsapp-client.ts`: Integração Evolution API.
- `jobs/send-whatsapp.ts`: Worker do BullMQ para processamento de fila.
- `src/app/(admin)/admin/campanhas/page.tsx`: Dashboard de campanhas.
- `src/components/admin/campanha-wizard.tsx`: UI de criação.
...
- [x] Instalar BullMQ e nanoid
- [x] Adicionar modelos Campanha e TrackingLink
- [x] Implementar tracking-service.ts
- [x] Implementar campanha-service.ts
- [x] Implementar whatsapp-client.ts
- [x] Criar worker BullMQ
- [x] Implementar API routes
- [x] Criar wizard de campanha
- [x] Criar página de detalhes
- [x] Implementar hooks
- [x] Testar envio completo
