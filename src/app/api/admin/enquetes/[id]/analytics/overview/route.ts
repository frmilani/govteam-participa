...
import { getOverview } from "@/lib/analytics/analytics-service";
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (!enquete) return NextResponse.json({ error: "Enquete não encontrada ou acesso restrito pelo seu perfil." }, { status: 404 });
